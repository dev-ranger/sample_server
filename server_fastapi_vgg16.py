...
cat_path = "./sample_data/cat_224x224.jpg"
jellyfish_path = "./sample_data/jellyfish.jpg"
...
async def get_label_prediction():
    try:
        path = jellyfish_path
        result = await vgg16_prediction_model.prediction_model(vgg16_model,path,True)
...
async def get_score_prediction():
    try:
        path = jellyfish_path
        result = await vgg16_prediction_model.prediction_model(vgg16_model,path,False)
        logger.info("Prediction was requested and done")
        return result
    except Exception as e:
        logger.error("Prediction failed: %s", e)
